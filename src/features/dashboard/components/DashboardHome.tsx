  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        <h1 className={`${tw.heading} ${tw.textPrimary} flex items-center gap-3`}>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <p className={`text-4xl font-bold ${tw.textPrimary}`}>{stat.value}</p>
                    <p className={`${tw.caption} font-medium ${tw.textSecondary}`}>{stat.name}</p>
                  </div>
                ...
                {/* Footer indicator */}
                <div className={`text-xs ${tw.textMuted} pt-3 border-t border-gray-100`}>
                  vs last month
                </div>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1 min-w-0">
                          <h3 className={`font-bold ${tw.textPrimary} text-lg mb-2`}>
                            {campaign.name}
                          </h3>
                          <div className="flex items-center gap-2">
                            <Users className={`h-4 w-4 ${tw.textMuted}`} />
                            <p className={`text-sm ${tw.textSecondary} font-medium`}>
                        ...
                        <div className="flex items-center gap-3 flex-shrink-0">
                          <span
                            className={`px-3 py-1.5 rounded-full text-xs font-bold ${campaign.status === 'active'
                      ...
                      {campaign.status === 'active' && campaign.performance ? (
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          {[
                            { label: 'Response', value: campaign.performance.response, icon: MessageSquare },
                            { label: 'Delivered', value: campaign.performance.delivered, icon: TrendingUp },
                            { label: 'Converted', value: campaign.performance.converted, icon: Target }
                          ].map((metric) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  <MetricIcon className="h-4 w-4" style={{ color: color.primary.accent }} />
                                </div>
                                <p className={`text-2xl font-bold ${tw.textPrimary}`}>
                                  {metric.value.toLocaleString()}
                                </p>
                                <p className={`text-xs ${tw.textSecondary} font-medium mt-1`}>
                                  {metric.label}
                                </p>
                              </div>
                            );
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="rounded-xl p-4 border border-gray-200 bg-gray-50">
                <p className="font-bold text-sm text-gray-900 mb-1">Campaign Launch</p>
                <p className="text-sm text-gray-600 mb-3">Churn Prevention - Q1</p>
              ...
              <div className="rounded-xl p-4 border border-gray-200 bg-gray-50">
                <p className="font-bold text-sm text-gray-900 mb-1">Review Meeting</p>
                <p className="text-sm text-gray-600 mb-3">Q1 Campaign Performance</p>
                <div className="inline-block px-3 py-1.5 rounded-full text-xs font-bold bg-gray-200 text-gray-700">
                  Friday at 2:00 PM
                </div>
              </div>
