...
export default function ListUpload({
  onListDataChange,
  listData,
  hideExistingLists = false,
  showCreateButton = false,
}: ListUploadProps) {
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [mode, setMode] = useState<"existing" | "new">(
    listData?.mode || "existing"
  );
  ...
  return (
    <div
      className="space-y-4 p-4 rounded-md border border-gray-200"
      ...
      {!hideExistingLists && (
        <div className="flex space-x-2 mb-4">
          <button
            ...
            className="flex items-center px-4 py-2 rounded-md font-medium transition-colors text-white"
            style={{
              backgroundColor:
                mode === "existing" ? color.primary.action : "transparent",
          ...
          <button
            ...
            style={{
              backgroundColor:
                mode === "new" ? color.primary.action : "transparent",
          ...
          {selectedList && (
            ...
            >
              <p className="text-sm" style={{ color: color.status.success }}>
                <strong>Selected:</strong> {selectedList.name} (
                {selectedList.subscriber_count.toLocaleString()} subscribers)
              </p>
            </div>
          )}
        </div>
            ...
            {!uploadedFile ? (
              <div
                ...
                onMouseEnter={(e) => {
                  ...
                  ).style.backgroundColor = `${color.primary.action}05`;
                }}
                onMouseLeave={(e) => {
                  (e.target as HTMLDivElement).style.borderColor =
                    tw.borderDefault;
