...
class OfferService {
  ...
  async getTypeDistribution(skipCache: boolean = false): Promise<TypeDistributionResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<TypeDistributionResponse>(`/type-distribution${params}`);
  }

  async getCategoryPerformance(skipCache: boolean = false): Promise<CategoryPerformanceResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<CategoryPerformanceResponse>(`/category-performance${params}`);
  }
  ...
  async getPendingApprovalOffers(params: FilterParams = {}): Promise<OffersResponse> {
    const queryParams = new URLSearchParams();
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.offset) queryParams.append('offset', params.offset.toString());
    if (params.skipCache) queryParams.append('skipCache', 'true');
    
    return this.request<OffersResponse>(`/pending-approval?${queryParams.toString()}`);
  }
  ...
  async getOffersByDateRange(params: DateRangeParams): Promise<OffersResponse> {
    ...
    return this.request<OffersResponse>(`/date-range?${queryParams.toString()}`);
  }

  // Specific Lookups 
  async getOffersByStatus(status: OfferStatusEnum, params: FilterParams = {}): Promise<OffersResponse> {
    const queryParams = new URLSearchParams();
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.offset) queryParams.append('offset', params.offset.toString());
    if (params.skipCache) queryParams.append('skipCache', 'true');
    
    return this.request<OffersResponse>(`/status/${status}?${queryParams.toString()}`);
  }

  async getOffersByType(type: OfferTypeEnum, params: FilterParams = {}): Promise<OffersResponse> {
    const queryParams = new URLSearchParams();
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.offset) queryParams.append('offset', params.offset.toString());
    if (params.skipCache) queryParams.append('skipCache', 'true');
    
    return this.request<OffersResponse>(`/type/${type}?${queryParams.toString()}`);
  }

  async getOffersByCategory(categoryId: number, params: FilterParams = {}): Promise<OffersResponse> {
    const queryParams = new URLSearchParams();
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.offset) queryParams.append('offset', params.offset.toString());
    if (params.skipCache) queryParams.append('skipCache', 'true');
    
    return this.request<OffersResponse>(`/category/${categoryId}?${queryParams.toString()}`);
  }

  async getOffersByProduct(productId: number, params: FilterParams = {}): Promise<OffersResponse> {
    const queryParams = new URLSearchParams();
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.offset) queryParams.append('offset', params.offset.toString());
    if (params.skipCache) queryParams.append('skipCache', 'true');
    
    return this.request<OffersResponse>(`/product/${productId}?${queryParams.toString()}`);
  }

  async getOfferByUuid(uuid: string, skipCache: boolean = false): Promise<OfferResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<OfferResponse>(`/uuid/${uuid}${params}`);
  }

  async getOfferByName(name: string, skipCache: boolean = false): Promise<OfferResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<OfferResponse>(`/name/${encodeURIComponent(name)}${params}`);
  }

  async getOfferByCode(code: string, skipCache: boolean = false): Promise<OfferResponse> {
    const params = skipCache ? '?skipCache=true' : '';
  ...
  async updateOfferStatus(id: number, request: UpdateStatusRequest): Promise<OfferResponse> {
  ...
  async submitForApproval(id: number, request: SubmitApprovalRequest): Promise<OfferResponse> {
  ...
  async approveOffer(id: number, request: ApproveOfferRequest): Promise<OfferResponse> {
  ...
  // Nested Resources 
  async getOfferProducts(id: number, skipCache: boolean = false): Promise<OfferProductsResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<OfferProductsResponse>(`/${id}/products${params}`);
  }

  // Basic CRUD 
  async getOfferById(id: number, skipCache: boolean = false): Promise<OfferResponse> {
    const params = skipCache ? '?skipCache=true' : '';
    return this.request<OfferResponse>(`/${id}${params}`);
  }

  async updateOffer(id: number, offer: UpdateOfferRequest): Promise<OfferResponse> {
