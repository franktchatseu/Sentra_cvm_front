...
export default function ProductTypesPage() {
  ...
  const handleDeleteProductType = async (productType: ProductType) => {
    ...
    try {
      setProductTypes((prev) =>
        prev.filter((type) => type.id !== productType.id)
      );
      success(
        "Product Type Deleted",
        `"${productType.name}" has been deleted successfully.`
      );
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className={`${tw.textSecondary} mt-2 text-sm`}>
              Manage different types of products in your catalog
            </p>
          </div>
        </div>
      ...
      {/* Product Types Table */}
      <div
        className={`bg-white rounded-lg border border-[${tw.borderDefault}] overflow-hidden`}
      >
