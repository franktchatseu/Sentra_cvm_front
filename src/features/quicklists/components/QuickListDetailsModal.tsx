...
export default function QuickListDetailsModal({
  ...
  const [currentQuickList, setCurrentQuickList] = useState<QuickList>(quicklist);
  ...
  const loadQuickListDetails = async () => {
    try {
      const response = await quicklistService.getQuickListById(quicklist.id, true);
  ...
  const loadData = async () => {
    try {
      setLoadingData(true);
      const response = await quicklistService.getQuickListData(quicklist.id, { limit: 50 });
      ...
      if (response.data && response.data.length > 0) {
        ...
        const extractedColumns = Object.keys(firstRow).filter(
          key => !['id', 'quicklist_id', 'created_at', 'row_number'].includes(key)
        );
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex-1">
            <h2 className={`${tw.subHeading} text-gray-900`}>{currentQuickList.name}</h2>
            {currentQuickList.description && (
              <p className="text-sm text-gray-500 mt-1">{currentQuickList.description}</p>
            )}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-gray-50 border-b">
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900">{currentQuickList.upload_type}</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900">
                {currentQuickList.row_count != null ? currentQuickList.row_count.toLocaleString() : 'N/A'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900">{formatDate(currentQuickList.created_at)}</p>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900">{currentQuickList.created_by}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-b">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <span className="font-medium text-gray-900">{currentQuickList.file_name || 'N/A'}</span>
            </div>
            <div>
              <span className="text-gray-500">File Size:</span>{' '}
              <span className="font-medium text-gray-900">
                {currentQuickList.file_size != null ? formatFileSize(currentQuickList.file_size) : 'N/A'}
              </span>
            </div>
            <div>
              <span className="text-gray-500">Columns:</span>{' '}
              <span className="font-medium text-gray-900">{currentQuickList.column_count ?? 'N/A'}</span>
            </div>
          </div>
        </div>
              ...
              <div className="overflow-x-auto">  
                <table className="w-full text-sm">
                  <thead className="bg-gray-800 border-b border-gray-700">
                    <tr>
                      ...
                      {(currentQuickList.columns || []).slice(0, 10).map((column) => (
                        <th
                          key={column}
                      ...
                      >
                        ...
                        {(currentQuickList.columns || []).slice(0, 10).map((column) => (
                          <td key={column} className="px-4 py-3 text-gray-600">
                            {(row as any)[column] !== undefined && (row as any)[column] !== null
                ...
                {(currentQuickList.columns?.length || 0) > 10 && (
                  <p className="text-xs text-gray-500 mt-4 text-center">
                    Showing first 10 columns of {currentQuickList.columns?.length || 0} total columns
                  </p>
                )}
              </div>
            )}
