...
export default function MultipleTargetOfferMapping({
  ...
  const handleOfferSelect = (offers: CampaignOffer[]) => {
    ...
    offers.forEach(offer => {
      ...
      const existingMappingIndex = segmentOfferMappings.findIndex(
        m => m.segment_id === editingSegmentId && m.offer_id === parseInt(offer.id)
      );
  ...
  const handleRemoveMapping = (segmentId: string, offerId: number) => {
    ...
    if (!offerUsedElsewhere) {
      setSelectedOffers(selectedOffers.filter(o => parseInt(o.id) !== offerId));
    }
  };

  const getOffersForSegment = (segmentId: string): CampaignOffer[] => {
    const mappings = segmentOfferMappings.filter(m => m.segment_id === segmentId);
    return mappings
              ...
              <div key={segment.id} className={components.card.surface}>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className={`text-sm font-medium ${tw.textPrimary}`}>{segment.name}</h4>
                ...
                {/* Mapped Offers */}
                <div>
                  <div className={`text-xs font-medium ${tw.textSecondary} mb-2`}>
                    Mapped Offers:
                  </div>
                  {mappedOffers.length === 0 ? (
                    <div className="text-center py-4 border-2 border-dashed rounded-lg" style={{ borderColor: color.border.default }}>
                      <p className={`text-xs ${tw.textSecondary}`}>No offers mapped to this segment</p>
                    </div>
                  ) : (
                      ...
                      {mappedOffers.map((offer) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            >
                              <Gift className="w-4 h-4" style={{ color: color.tertiary.tag1 }} />
                            </div>
                            <div>
                              <div className={`text-sm font-medium ${tw.textPrimary}`}>{offer.name}</div>
                              <div className={`text-xs ${tw.textSecondary}`}>
                                {offer.reward_value} • {offer.validity_period}d validity
                              </div>
                            </div>
                          </div>
                          <button
                            onClick={() => handleRemoveMapping(segment.id, parseInt(offer.id))}
