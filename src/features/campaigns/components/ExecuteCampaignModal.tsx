...
import { X, Play, AlertTriangle, Mail, MessageSquare, Bell } from 'lucide-react';
...
export default function ExecuteCampaignModal({
  ...
  const loadSegments = async () => {
    setIsLoading(true);
    try {
      const response = await campaignSegmentOfferService.getMappingsByCampaign(campaignId);
  ...
  const handleExecute = async () => {
    const selectedSegments = segments.filter(s => s.selected && s.channels.length > 0);
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b" style={{ borderColor: color.border.default }}>
            <div className="flex items-center gap-3">
              ...
              >
                <Play className="w-5 h-5" style={{ color: color.primary.accent }} />
              </div>
              <div>
                <h2 className={`text-xl font-semibold ${tw.textPrimary}`}>Execute Campaign</h2>
                <p className={`text-sm ${tw.textSecondary} mt-0.5`}>{campaignName}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-6">
            {/* Warning Alert */}
            <div className="flex items-start gap-3 p-4 rounded-lg" style={{ backgroundColor: '#FEF3C7', border: '1px solid #FCD34D' }}>
              <AlertTriangle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
              <div>
                <h4 className="text-sm font-medium text-amber-900">Confirm Campaign Execution</h4>
                <p className="text-sm text-amber-700 mt-1">
                  This will send communications to the selected segments. Please review carefully before proceeding.
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  style={executionMode === 'immediate' ? { borderColor: color.primary.accent } : {}}
                >
                ...
                <button
                  ...
                  style={executionMode === 'scheduled' ? { borderColor: color.primary.accent } : {}}
                >
                  <div className="font-medium text-sm">Scheduled</div>
                  <div className="text-xs text-gray-500 mt-1">Execute as scheduled</div>
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              {isLoading ? (
                <div className="text-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 mx-auto" style={{ borderColor: color.primary.accent }}></div>
                  <p className={`text-sm ${tw.textSecondary} mt-2`}>Loading segments...</p>
                </div>
              ) : segments.length === 0 ? (
                ...
                <div className="space-y-3">
                  {segments.map((segment) => (
                    <div key={segment.id} className={`${components.card.surface} transition-all ${segment.selected ? 'border-2' : ''}`} style={segment.selected ? { borderColor: color.primary.accent } : {}}>
                      ...
                      {/* Channels */}
                      {segment.selected && (
                        <div className="mt-3 pt-3 border-t" style={{ borderColor: color.border.default }}>
                          <div className="text-xs font-medium text-gray-600 mb-2">Channels:</div>
                          <div className="flex flex-wrap gap-2">
                            {AVAILABLE_CHANNELS.map((channel) => {
                              const Icon = channel.icon;
                              const isSelected = segment.channels.includes(channel.code);
                              return (
                                <button
                                  key={channel.code}
                                  onClick={() => toggleChannel(segment.segment_id, channel.code)}
                                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm transition-all border-2 ${
                                    isSelected ? '' : 'hover:bg-gray-100'
                                  }`}
                                  style={
                                    isSelected
                                      ? { backgroundColor: `${channel.color}20`, color: channel.color, borderColor: channel.color }
                                      : { backgroundColor: color.surface.cards, borderColor: 'transparent' }
                                  }
                                >
          ...
          <div className="flex items-center justify-end gap-3 p-6 border-t" style={{ borderColor: color.border.default }}>
            ...
            <button
              ...
              disabled={isExecuting || segments.filter(s => s.selected && s.channels.length > 0).length === 0}
